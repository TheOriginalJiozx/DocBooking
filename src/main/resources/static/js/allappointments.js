    ...
    fetch(`/api/admin/appointments`)
        ...
        .then(data => {
            if (!data || data.length === 0) {
                document.getElementById('appointments-container').innerHTML = "No appointments found.";
                return;
            }
...
async function rescheduleAppointment(event, appointmentId, doctorName) {
    ...
    try {
        ...
        if (rescheduleResponse.ok) {
            const successMessage = "The appointment has been rescheduled!";
            showResponse(successMessage);
        ...
        showError("Error rescheduling the appointment: " + error.message);
    }
}
